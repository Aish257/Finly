...
def ask_ai_coach(user_query, context=""):
    """
    Asks Gemini 2.5 Flash for financial coaching response, falling back to local engine if needed.
    """
    ...
    if client_tuple:
        ...
        try:
            if sdk_type == "new_sdk":
                ...
                model = client_obj.GenerativeModel("gemini-2.5-flash")
                response = model.generate_content(full_prompt)
                if response and hasattr(response, 'text') and response.text:
                    return response.text.strip()
        except Exception as e:
            # Fall back gracefully on API rate-limit, invalid key, or network issue
            pass
    ...
    for key, response_text in FALLBACK_RESPONSES.items():
        if key in query_lower:
            return f"💡 **Finly Assistant (Offline Mode)**:\n\n{response_text}"
